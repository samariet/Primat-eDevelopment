...
#dat_cpm <- cpm(both_minus, lib.size = colSums(both_minus) * #calcNormFactors(both_minus),log = TRUE)
...
gene_names <- getBM(filters= "ensembl_gene_id", attributes= c("ensembl_gene_id",
"hgnc_symbol"),values=rownames(gene_prob_day),mart= mart)
...
prob_sp_2 <- rownames(gene_prob_day[(gene_prob_day[,1] >0.8 & gene_prob_day[,2] >0.8 & gene_prob_day[,3] <0.5 & gene_prob_day[,4] <0.5),])
length(prob_sp_2)

prob_sp_3 <- rownames(gene_prob_day[(gene_prob_day[,1] >0.8 & gene_prob_day[,2] >0.8 & gene_prob_day[,3] > 0.8 & gene_prob_day[,4] > 0.8),])
length(prob_sp_3)

prob_sp_4 <- rownames(gene_prob_day[(gene_prob_day[,1] <0.5& gene_prob_day[,2] <0.5 & gene_prob_day[,3] <0.5 & gene_prob_day[,4] >0.8),])
length(prob_sp_4)

prob_sp_5 <- rownames(gene_prob_day[(gene_prob_day[,1] <0.5 & gene_prob_day[,2] <0.5 & gene_prob_day[,3] > 0.8 & gene_prob_day[,4] >0.8),])
length(prob_sp_5)
...
rownames(gene_prob_tran) <- rownames(dat_cpm_trimmed)
...
prob_day_1  <- rownames(gene_prob_tran[(gene_prob_tran[,1] >0.9 & gene_prob_tran[,2] <0.5 & gene_prob_tran[,3] <0.5 & gene_prob_tran[,4] >0.9 & gene_prob_tran[,5] < 0.5 & gene_prob_tran[,6]<0.5),])
length(prob_day_1)
write.table(prob_day_1, file="prob_day_1.txt")

prob_day_2  <- rownames(gene_prob_tran[(gene_prob_tran[,1] <0.5 & gene_prob_tran[,2] <0.5 & gene_prob_tran[,3] >0.9 & gene_prob_tran[,4] < 0.5 & gene_prob_tran[,5] < 0.5 & gene_prob_tran[,6]>0.9),])
length(prob_day_2)
write.table(prob_day_2, file="prob_day_2.txt")


prob_day_3  <- rownames(gene_prob_tran[(gene_prob_tran[,1] <0.5 & gene_prob_tran[,2] >0.9 & gene_prob_tran[,3] >0.9 & gene_prob_tran[,4] < 0.5 & gene_prob_tran[,5] >0.9 & gene_prob_tran[,6]>0.9),])
length(prob_day_3)
write.table(prob_day_3, file="prob_day_3.txt")


prob_clust_5  <- rownames(gene_prob_tran[(gene_prob_tran[,1] >0.5 & gene_prob_tran[,2] >0.9 & gene_prob_tran[,3] >0.9 & gene_prob_tran[,4] < 0.7 & gene_prob_tran[,5] <0.5& gene_prob_tran[,6]<0.5),])
length(prob_clust_5)
write.table(prob_clust_5, file="prob_clust_5.txt")
...
rownames(gene_prob_all) <- rownames(dat_cpm_trimmed)                            
plotIC(cormotif_all)
...
rownames(gene_prob_chimp) <- rownames(dat_cpm_trimmed)                            
...
rownames(gene_prob_human) <- rownames(dat_cpm_trimmed)
...
#counts <- read.delim("endoderm-counts-filtered.txt")
#counts[1:10, 1:5]
#anno <- read.delim("samplefactors-filtered.txt", stringsAsFactors = FALSE)
#stopifnot(rownames(anno) == colnames(counts))
#day <- day
...
ggplot(variance, aes(x=var, fill=day)) + geom_density(alpha=0.5)+xlab("Variance") + theme(legend.position=c(.75,.75), legend.title=element_blank(), axis.title=element_text(size=14), panel.background=element_rect(fill='white')) + theme(text = element_text(size=18)) +xlim(c(-0.1,1))
...
ggplot(variance_humans, aes(x=var, fill=day)) + geom_density(alpha=0.5) +xlim(-.25,8.0)+xlab("Variance") + theme(legend.position=c(.75,.75), legend.title=element_blank(), axis.title=element_text(size=14), panel.background=element_rect(fill='white')) + theme(text = element_text(size=18)) +xlim(-.01, 2)
...
ggplot(variance_chimps, aes(x=var, fill=day)) + geom_density(alpha=0.5) +xlim(-.25,8.0)+xlab("Variance") + theme(legend.position=c(.75,.75), legend.title=element_blank(), axis.title=element_text(size=14), panel.background=element_rect(fill='white')) + theme(text = element_text(size=18)) +xlim(-.01, 2)

t.test(var_d2C, var_d3C)
...
counts <- read.delim("endoderm-counts-filtered.txt")
counts[1:10, 1:5]
sample_features <- read.delim("samplefactors-filtered.txt", stringsAsFactors = FALSE)
stopifnot(rownames(sample_features) == colnames(counts))
...
rownames(sample_features) <- colnames(dat_cpm)
pair_cor_long$day1 <- ""
pair_cor_long$day2 <- ""
pair_cor_long$species1 <- ""
pair_cor_long$species2 <- ""
for (i in 1:nrow(pair_cor_long)) {
  pair_cor_long$day1[i] <- sample_features[pair_cor_long$sample1[i], "Day"]
  pair_cor_long$day2[i] <- sample_features[pair_cor_long$sample2[i], "Day"]
  pair_cor_long$species1[i] <- sample_features[pair_cor_long$sample1[i], "Species"]
  pair_cor_long$species2[i] <- sample_features[pair_cor_long$sample2[i], "Species"]
}
head(pair_cor_long)
...
ggplot(df_plot, aes(x = day1, y = correlation)) +
  ...
  labs(x = "Day", y = "Pearson's r", title = "Evidence of canalization?")
...
ggplot(effect_size, aes(x=ef, fill=day)) + geom_density(alpha=0.5) +xlim(-.25,8.0)+xlab("Effect_size") + theme(legend.position=c(.75,.75), legend.title=element_blank(), axis.title=element_text(size=14), panel.background=element_rect(fill='white')) + theme(text = element_text(size=18))  
#+scale_fill_manual(values=cols, labels=c("LCLs", "iPSCs"))
...
ggplot(effect_size_intrxn, aes(x=ef, fill=day)) + geom_density(alpha=0.5) +xlim(-.25,8.0)+xlab("Effect_size") + theme(legend.position=c(.75,.75), legend.title=element_blank(), axis.title=element_text(size=14), panel.background=element_rect(fill='white')) + theme(text = element_text(size=18))  +xlim(c(-0.1,5))
...
output_all_3 <- topTable(fit2, coef="D3", number=20000, sort.by="none")
head(output_all_0)
mean(abs(output_all_0$logFC))

dfs <- data.frame(output_all_0$B, output_all_1$B, output_all_2$B, output_all_3$B)
for(i in 1:nrow(df.norm)){df.norm[i,]=df.norm[i,]/max(abs(df.norm[i,]))}
dfs <- stack(dfs)
colnames(df.norm)=c("d1","d2","d3","d4")
heatmap(as.matrix(df.norm[1:1000,]))
head(dfs)
df.norm.hclust=hclust(as.matrix(df.norm[1:1000,]))
plot(density((FC_all)))
...
ggplot(effect_size_spday, aes(x=ef, fill=day)) + geom_density(alpha=0.5)+xlab("Effect_size") + theme(legend.position=c(.75,.75), legend.title=element_blank(), axis.title=element_text(size=14), panel.background=element_rect(fill='white')) + theme(text = element_text(size=18)) +xlim(c(-0.1,2))
t.test(BspeciesD2,BspeciesD3)
...
mean_effect_D0x <- mean(abs(output_D0$logFC))
mean_effect_D1x <- mean(abs(output_D1$logFC))
mean_effect_D2x <- mean(abs(output_D2$logFC))
mean_effect_D3x <- mean(abs(output_D3$logFC))

mean_effectsx <- c(mean_effect_D0x, mean_effect_D1x, mean_effect_D2x, mean_effect_D3x)
plot(mean_effectsx)

effect_size_spday <- data.frame(ef=c(abs(output_D0$B), abs(output_D1$B), abs(output_D2$B), abs(output_D3$B)), day = rep(c("D0", "D1", "D2", "D3"), times =c(length(output_D0$logFC), length(output_D1$logFC), length(output_D2$logFC), length(output_D3$logFC))))

ggplot(effect_size_spday, aes(x=ef, fill=day)) + geom_density(alpha=0.5)+xlab("Effect_size") + theme(legend.position=c(.75,.75), legend.title=element_blank(), axis.title=element_text(size=14), panel.background=element_rect(fill='white')) + theme(text = element_text(size=18)) +xlim(c(-0.1,5))
...
par(mfrow=c(1,1))
venn.diagram(list(A=sig_d0, B=sig_d1, C=sig_d2, D=sig_d3), "Venn.tiff")
...
prob_sp_1 <- dat_cpm_trimmed[prob_sp_1,]
prob_sp_1<- prob_sp_1[rowMeans(prob_sp_1)>2,]
for (i in 10:20) {
play <- prob_sp_1[i,]
playdf <- data.frame(play,day,species)
#print(playdf)
print(ggplot(data=playdf, aes(x=day, y=play, fill=species)) + geom_boxplot(aes(fill=species)) + ggtitle(rownames(prob_sp_1)[i]))
}


prob_sp_3 <- dat_cpm_trimmed[prob_sp_3,]
prob_sp_3<- prob_sp3[rowMeans(prob_sp_3)>2,]
for (i in 10:20) {
play <- prob_sp_3[i,]
playdf <- data.frame(play,day,species)
#print(playdf)
print(ggplot(data=playdf, aes(x=day, y=play, fill=species)) + geom_boxplot(aes(fill=species)) + ggtitle(rownames(prob_sp_3)[i]))
}


prob_sp_2 <- dat_cpm_trimmed[prob_sp_2,]
prob_sp_2<- prob_sp_2[rowMeans(prob_sp_2)>2,]
for (i in 10:20) {
play <- prob_sp_2[i,]
playdf <- data.frame(play,day,species)
#print(playdf)
print(ggplot(data=playdf, aes(x=day, y=play, fill=species)) + geom_boxplot(aes(fill=species)) + ggtitle(rownames(prob_sp_2)[i]))
}


prob_sp_4 <- dat_cpm_trimmed[prob_sp_4,]
prob_sp_4<- prob_sp_4[rowMeans(prob_sp_4)>2,]
for (i in 10:20) {
play <- prob_sp_4[i,]
playdf <- data.frame(play,day,species)
#print(playdf)
print(ggplot(data=playdf, aes(x=day, y=play, fill=species)) + geom_boxplot(aes(fill=species)) + ggtitle(rownames(prob_sp_4)[i]))
}


prob_sp_5 <- dat_cpm_trimmed[prob_sp_5,]
prob_sp_5<- prob_sp_5[rowMeans(prob_sp_5)>2,]
for (i in 10:20) {
play <- prob_sp_5[i,]
playdf <- data.frame(play,day,species)
#print(playdf)
print(ggplot(data=playdf, aes(x=day, y=play, fill=species)) + geom_boxplot(aes(fill=species)) + ggtitle(rownames(prob_sp_5)[i]))
}

###Specific Genes###

tfs <- unlist(read.table("TFSensg.txt"))

tfs_dat <- dat_cpm[rownames(dat_cpm) %in% tfs,]
gene_names <- getBM(filters= "ensembl_gene_id", attributes= c("ensembl_gene_id","hgnc_symbol"),values=rownames(tfs_dat),mart= ensembl)
tfs_dat <- tfs_dat[rownames(tfs_dat) %in% gene_names[,1],]
all(rownames(tfs_dat) == gene_names[,1])
rownames(tfs_dat) <- gene_names[,2]
rownames(tfs_dat)[12] <- "BRACHYURY"
heatmap(tfs_dat)
...
expr_tmem65 <- dat_cpm_trimmed["ENSG00000164983", ]
tmem65 <- data.frame(expr_tmem65,day,species)
ggplot(data=tmem65, aes(x=day, y=expr_tmem65, fill=species)) + geom_boxplot(aes(fill=species)) + ylab("expression TMEM65")


expr_apoe <- dat_cpm_trimmed["ENSG00000130203", ]
apoe <- data.frame(expr_apoe,day,species)
ggplot(data=apoe, aes(x=day, y=expr_apoe, fill=species)) + geom_boxplot(aes(fill=species)) + ggtitle("APOE")+ 
  ylab("expression of ApoE")
...
foxh1<- dat_cpm_trimmed["ENSG00000160973", ]
foxh1<- data.frame(foxh1,day,species)
ggplot(data=foxh1, aes(x=day, y=foxh1, fill=species)) + geom_boxplot(aes(fill=species)) + ylab("FoxH1 Expression"
)
...
wnt3a<- dat_cpm_trimmed["ENSG00000108379", ]
wntdf<- data.frame(wnt3a,day,species)
ggplot(data=wntdf, aes(x=day, y=wnt3a, fill=species)) + geom_boxplot(aes(fill=species)) + ylab("Wnt3a Expression")

bra<- dat_cpm_trimmed["ENSG00000164458", ]
bradf<- data.frame(bra,day,species)
ggplot(data=bradf, aes(x=day, y=bra, fill=species)) + geom_boxplot(aes(fill=species)) + ylab("Brachyury Expression")

sox2<- dat_cpm_trimmed["ENSG00000075388", ]
sox2df<- data.frame(sox2,day,species)
ggplot(data=sox2df, aes(x=day, y=sox2, fill=species)) + geom_boxplot(aes(fill=species)) + ggtitle("SOX2")
...
axis(side=1, at = c(0.5, 2.5, 2.5, 3.5, 4.5, 5.5), tick=FALSE, labels=c("Day 0", "Day 1", "Day 2", "Day 3", "never DE", "always DE"))
...
length(sig_all)
collect_total_barplot <- c((sig_day0-sig_day0_only), (sig_day1-sig_day1_only), (sig_day2-sig_day2_only), (sig_day3-sig_day3_only), Number_notsig, always_sig)
...
barplot(msig, ylab="Number")
axis(side=1, at = c(1.2 ,2.4, 2.5, 3.5, 4.5, 5.5), tick=FALSE, labels=c("Day 0", "Day 1", "Day 2", "Day 3", "never DE", "always DE"))
...
Master <- c("ENSG00000111704", ENSG0000181449, ENSG00000079313, ENSG00000125798)
boxplot(var_d0H, var_d0C, var_d1H, var_d1C, var_d2H, var_d2C, var_d3H, var_d3C, ylim=c(0,4))
#table of all
...
gds3904 <- getGEO(filename='GDS3904.soft')
eset <- GDS2eSet(gds3904, do.log2=TRUE)
match <- exprs(eset[])