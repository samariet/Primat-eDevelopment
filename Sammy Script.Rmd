...
for (i in 1:nrow(pair_cor_long)) {
  ...
  if (pair_cor_long$species1[i] == "2" &
      pair_cor_long$species2[i] == "2") {
    pair_cor_long$human[i] <- pair_cor_long$r[i]
  } else if (pair_cor_long$species1[i] == "1" &
             pair_cor_long$species2[i] == "1") {
...
ensembl <- useMart(host = "sep2013.archive.ensembl.org",
                   biomart = "ENSEMBL_MART_ENSEMBL",
                   dataset = "hsapiens_gene_ensembl")
gene_names <- getBM(filters= "ensembl_gene_id", attributes= c("ensembl_gene_id","hgnc_symbol"),values=rownames(output_all),mart= ensembl)
output_all<- output_all[gene_names[,1] %in% rownames(output_all),]
all(rownames(output_all) == gene_names[,1])
rownames(output_all) == gene_names[,1]
rownames(output_all) <- gene_names[,2]
