...
#dat_cpm <- cpm(both_minus, lib.size = colSums(both_minus) * #calcNormFactors(both_minus),
                  log = TRUE)
...
effect_size_spday <- data.frame(ef=c(abs(output_all_0$B), abs(output_all_1$B), abs(output_all_2$B), abs(output_all_3$B)), day = rep(c("D0", "D1", "D2", "D3"), times =c(length(output_all_0$logFC), length(output_all_1$logFC), length(output_all_2$logFC), length(output_all_3$logFC))))
...
effect_size_spday <- data.frame(ef=c(abs(output_D0$logFC), abs(output_D1$logFC), abs(output_D2$logFC), abs(output_D3$logFC)), day = rep(c("D0", "D1", "D2", "D3"), times =c(length(output_D0$logFC), length(output_D1$logFC), length(output_D2$logFC), length(output_D3$logFC))))
